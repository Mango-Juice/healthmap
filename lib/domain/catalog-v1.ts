...
export const ProductionMenuObjectSchema = z
  .object({
    ...MenuFields,
    schemaVersion: z.literal("1.0.0").optional(),
    dataMode: z.literal("production"),
...
export const enforceMenuVerification = (
  menu: Pick<
    z.infer<typeof ProductionMenuObjectSchema>,
    "verificationMethod" | "evidenceUrl" | "verifiedAt" | "validUntil"
  >,
  context: z.RefinementCtx,
): void => {
...
export const ProductionMenuSchema =
  ProductionMenuObjectSchema.readonly().superRefine(enforceMenuVerification)
...
const toPlace = (row: RawPlace): Place => {
  const fields = {
    ...(row.schema_version === undefined ? {} : { schemaVersion: row.schema_version }),
    id: row.id,
    slug: row.slug,
    name: row.name,
    address: row.address,
