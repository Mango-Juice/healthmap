import { z } from "zod"
import { PILOT_DISCOVERY_FILTERS } from "./discovery"
...
export const PilotQuerySchema = z
  .strictObject({
    mode: z.enum(["places", "regions"]).default("places"),
    query: z.string().trim().max(200).default(""),
    filter: z.enum(PILOT_DISCOVERY_FILTERS.map((option) => option.value)).default("all"),
...
export type PilotQuery = z.infer<typeof PilotQuerySchema>
export const parsePilotQuery = (params: URLSearchParams) => {
  const entries = Array.from(params.entries())
  if (
    new Set(entries.map(([key]) => key)).size !== entries.length ||
    entries.some(([, value]) => value === "")
  )
    return PilotQuerySchema.safeParse({ invalid: true })
  return PilotQuerySchema.safeParse(Object.fromEntries(entries))
}
