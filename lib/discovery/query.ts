...
import type {
  DiscoveryPlaceResultDto,
  DiscoveryPlacesResponse,
  DiscoveryRegionsResponse,
} from "./dto"
import { filterDiscoveryPlaces, menusForDiscoveryPlace } from "./menu-selection"
import { toDiscoveryMenuDto, toDiscoveryPlaceDto, toSubwayStoreDto } from "./projection"
import type { DiscoveryQuery } from "./query-contract"
import { canonicalDiscoveryRegion } from "./region"
import {
  combinedDiscoveryCatalogVersion,
  type SubwayStore,
  type SubwayStoreCatalog,
} from "./subway"
...
const projectResult = (
  ...
  if (result.kind === "menu_evidence")
    return {
      place: toDiscoveryPlaceDto(result.place),
      matchingMenuIds: result.matchingMenuIds,
      menus: menusForDiscoveryPlace(catalog, result.place.id)
        .filter((menu) => result.matchingMenuIds.includes(menu.id))
        .sort((left, right) => left.id.localeCompare(right.id))
        .map(toDiscoveryMenuDto),
    }
  return {
...
export const queryDiscoveryCatalog = (
...
): DiscoveryPlacesResponse | DiscoveryRegionsResponse | DiscoveryQueryError => {
  ...
  const menuResults: readonly MenuEvidenceResult[] = filterDiscoveryPlaces({
    catalog,
    filter: query.filter,
  ...
  const matchingResults = [...menuResults, ...storeResults].filter(
    ({ place }) =>
      ...
      canonicalDiscoveryRegion(place.address) === canonicalDiscoveryRegion(query.region),
  )
  const catalogVersion = combinedDiscoveryCatalogVersion(catalog.catalogVersion, subway)
  if (query.mode === "regions") {
    const groups = new Map<string, typeof matchingResults>()
    for (const result of matchingResults) {
      const region = canonicalDiscoveryRegion(result.place.address)
