...
const menuPhoto = DiscoveryMediaSchema.parse({
...
const otherMenuPhoto = DiscoveryMediaSchema.parse({
  ...menuPhoto,
  url: "https://www.salady.com/data/menu/other.jpg",
  menuIds: ["22222222-2222-5222-8222-222222222222"],
})
const venuePhoto = DiscoveryMediaSchema.parse({
  ...menuPhoto,
...
describe("Discovery media selection", () => {
  it("selects the currently matching menu photo before a venue photo", () => {
    expect(
      selectDiscoveryMedia([venuePhoto, otherMenuPhoto, menuPhoto], [matchingMenuId])?.url,
    ).toBe(menuPhoto.url)
  })

  it("never selects a dish photo belonging to another matching result", () => {
    expect(selectDiscoveryMedia([otherMenuPhoto, venuePhoto], [matchingMenuId])?.url).toBe(
      venuePhoto.url,
    )
  })

  it("renders no media choice when only a wrong-menu dish exists", () => {
    expect(selectDiscoveryMedia([otherMenuPhoto], [matchingMenuId])).toBeUndefined()
  })
})
