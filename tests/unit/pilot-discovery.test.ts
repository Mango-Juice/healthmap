...
import { PilotMenuSchema } from "../../lib/pilot/catalog"
import {
...
describe("menu-first Pilot discovery", () => {
  it("Given ordinary meal candidates, When browsing or searching, Then unsupported choices stay out", () => {
    for (const query of ["일반", "합성", "국수", "한상"]) {
      const results = filterPilotPlaces({ catalog, filter: "all", query })
      expect(
        results.every((result) =>
          orderedResultMenus(catalog, result).every(hasPilotMealSelectionBasis),
        ),
      ).toBe(true)
    }
    const raw = catalog.menus.filter((menu) => menu.name.includes("일반"))
    expect(raw.length).toBeGreaterThan(0)
    expect(filterPilotPlaces({ catalog, filter: "rice", query: "일반" })).toEqual([])
    expect(
      filterPilotPlaces({ catalog, filter: "all", ingredient: "chicken", query: "" }).every(
        (result) => orderedResultMenus(catalog, result).every(hasPilotMealSelectionBasis),
      ),
    ).toBe(true)
  })
  it("Given a selected eligible meal, When detail lists other menus, Then it does not recommend unsupported dishes", () => {
    const [result] = filterPilotPlaces({ catalog, filter: "all", query: "합성 곡물" })
  ...
  it("keeps only meal choices with an explicit selection basis, independently of source", () => {
    ...
    for (const result of results) {
      ...
      expect(matching.every(hasPilotMealSelectionBasis)).toBe(true)
    }
  })
  it("shows the queried poke first even when the same place also has salad", () => {
    const results = filterPilotPlaces({ catalog, filter: "all", query: "합성 생선 포케" })
    expect(results.length).toBeGreaterThan(0)
    expect(
  ...
  it("never combines unrelated menus to satisfy a compound query", () => {
    ...
    const dish = PilotMenuSchema.parse({
      ...base,
      name: "닭국수",
      facts: {
        ...base.facts,
    ...
    const salad = PilotMenuSchema.parse({
      ...other,
      placeId: place.id,
      name: "두부샐러드",
      facts: {
    ...
    expect(filterPilotPlaces({ catalog: fixture, filter: "all", query: "닭 두부" })).toEqual([])
  })
  it("derives rice options only from menu evidence", () => {
    ...
    const results = filterPilotPlaces({ catalog, filter: "whole_grain", query: "" })
    expect(results.length).toBeGreaterThan(0)
    expect(
      results.every(
  ...
  it("uses current area for the same list and marker result", () => {
    const target = filterPilotPlaces({ catalog, filter: "all", query: "" })[0]?.place
  ...
  it("keeps SDK marker assets and selection emphasis", () => {
    expect(markerIconForMenus([], true)).toBe("/markers/pilot-neutral-selected.svg")
  ...
  it("uses the salad marker presentation for empty-menu Subway stores", () => {
    ...
    expect(markerIconForMenus([], false, "all", "subway")).toBe("/markers/pilot-salad_poke.svg")
  ...
  it("keeps the selected marker asset for a stable known-brand category", () => {
    ...
    expect(markerIconForMenus([salad], true, "all", "bon_dosirak")).toBe(
      "/markers/pilot-rice-selected.svg",
    )
  })
  it("labels every eligible cooking meal and keeps fish away from dietary pin symbols", () => {
    const eligible = catalog.menus.filter(hasPilotMealSelectionBasis)
    ...
    expect(pilotMenuFactLabel(fish)).toContain("생선 · 구이")
    ...
    expect(markerIconForMenus([fishSalad], false)).toBe("/markers/pilot-salad_poke.svg")
    for (const category of [
      ...
      const svg = readFileSync(resolve("public", pilotCategoryIcon(category).slice(1)), "utf8")
      expect(svg).toContain("<title>")
      expect(svg).not.toContain("비건")
    }
  })
  it("applies cooking category and ingredient to the same menu in API counts", () => {
    const query = PilotQuerySchema.parse({ filter: "grilled_steamed", ingredient: "fish" })
    const response = queryPilotCatalog(catalog, query)
  ...
  it("removes the redundant source prefix from menu display", () => {
    expect(presentPilotMenuName("[비건]포케")).toBe("포케")
  })
  it("distinguishes a source vegan label from an option that requires a change", () => {
    expect(pilotMenuDietaryNote("source_vegan_label")).toBe(
      "출처에서 비건 메뉴로 소개하고 있어요. 재료와 조리 방식은 주문할 때 확인해 주세요.",
    )
    expect(pilotMenuDietaryNote("vegan_option")).toContain("옵션 선택이나 변경이 필요")
  })
})
