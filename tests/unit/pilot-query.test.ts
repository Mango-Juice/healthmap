...
import { buildPilotRegionRequestUrl } from "../../components/pilot/use-pilot-query"
import { parsePilotQuery } from "../../lib/pilot/query-contract"

describe("Pilot query boundary", () => {
  it("rejects partial, inverted, nonfinite and unknown bounds", () => {
    for (const query of [
      ...
      expect(parsePilotQuery(new URLSearchParams(query)).success).toBe(false)
    }
  })
  it("defaults to fifty bounded places", () => {
    const result = parsePilotQuery(new URLSearchParams())
    expect(result.success && result.data.limit).toBe(50)
  })
})

describe("Pilot region request", () => {
...
describe("Pilot detail copy", () => {
  it("labels matching menus without asserting branch sale", () => {
    const source = readFileSync(
      new URL("../../components/pilot/pilot-detail.tsx", import.meta.url),
      "utf8",
    )
...
import { hasPilotMealSelectionBasis } from "../../lib/pilot/discovery"
...
import { type SubwayStore, SubwayStoreSchema } from "../../lib/pilot/subway"
import { syntheticPilotCatalog } from "../fixtures/pilot-catalog"

const catalog = syntheticPilotCatalog
const selectedMenu = catalog.menus.find((menu) => hasPilotMealSelectionBasis(menu))
...
const places = (search: string) => {
  const query = parsePilotQuery(new URLSearchParams(search))
  if (!query.success) throw new Error("invalid test query")
  return queryPilotCatalog(catalog, query.data)
}
const subwayCatalog = (stores: readonly SubwayStore[]) => ({
  version: "subway-test-snapshot",
  stores,
})
describe("server Pilot projection", () => {
  it("includes menu-free Subway locations only in browse and salad discovery", () => {
    ...
    const baseline = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(catalog, PilotQuerySchema.parse({ limit: 100 })),
    )
    const browse = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(catalog, PilotQuerySchema.parse({ limit: 100 }), subwayCatalog([store])),
    )
    const search = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(
        catalog,
        PilotQuerySchema.parse({ query: "서브웨이 부산", limit: 100 }),
        subwayCatalog([store]),
      ),
    )
    const salad = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(
        catalog,
        PilotQuerySchema.parse({ filter: "salad_poke", query: "서브웨이", limit: 100 }),
        subwayCatalog([store]),
      ),
    )
    const saladSearch = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(
        catalog,
        PilotQuerySchema.parse({ filter: "salad_poke", query: "서브웨이 샐러드", limit: 100 }),
        subwayCatalog([store]),
      ),
    )
    const saladArea = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(
        catalog,
        PilotQuerySchema.parse({
          filter: "salad_poke",
          south: 35.15,
          north: 35.17,
          west: 129.15,
    ...
    const saladRegions = PilotRegionsResponseSchema.parse(
      queryPilotCatalog(
        catalog,
        PilotQuerySchema.parse({ filter: "salad_poke", query: "서브웨이", mode: "regions" }),
        subwayCatalog([store]),
      ),
    )
    ...
    for (const ingredient of ["chicken", "fish", "tofu_soy"] as const) {
      const ingredientResults = PilotPlacesResponseSchema.parse(
        queryPilotCatalog(
          catalog,
          PilotQuerySchema.parse({ filter: "salad_poke", ingredient, limit: 100 }),
          subwayCatalog([store]),
        ),
      )
    ...
    for (const filter of ["grilled_steamed", "whole_grain", "plant_based", "rice"] as const) {
      const otherCategory = PilotPlacesResponseSchema.parse(
        queryPilotCatalog(
          catalog,
          PilotQuerySchema.parse({ filter, limit: 100 }),
          subwayCatalog([store]),
        ),
      )
  ...
  it("keeps store-only pagination, region recovery, and cursors coherent", () => {
    ...
    const first = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(
        catalog,
        PilotQuerySchema.parse({ filter: "salad_poke", query: "서브웨이", limit: 1 }),
        subwayCatalog(stores),
      ),
    )
    const second = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(
        catalog,
        PilotQuerySchema.parse({
          filter: "salad_poke",
  ...
  it("orders all matching places by normalized phrase relevance before distance", () => {
    ...
    const query = PilotQuerySchema.parse({ query: "샐러드 특별", limit: 100 })

    // When the complete result set is queried.
    const response = PilotPlacesResponseSchema.parse(queryPilotCatalog(orderingCatalog, query))
  ...
  it("orders blank-query results globally before pagination with stable ID ties", () => {
    ...
    const first = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(tieCatalog, PilotQuerySchema.parse({ limit: 1 })),
    )
    const second = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(tieCatalog, PilotQuerySchema.parse({ limit: 1, cursor: first.nextCursor })),
    )
  ...
  it("derives map and region anchors from the applied result scope", () => {
    ...
    const mapQuery = PilotQuerySchema.parse({
      south: -1,
      north: 1,
      west: -2,
      east: 2,
    ...
    const regionQuery = PilotQuerySchema.parse({ region: "서울 중구", limit: 100 })
    const emptyQuery = PilotQuerySchema.parse({ south: 40, north: 41, west: 130, east: 131 })

    // When each scope is queried.
    const map = PilotPlacesResponseSchema.parse(queryPilotCatalog(orderingCatalog, mapQuery))
    const region = PilotPlacesResponseSchema.parse(queryPilotCatalog(orderingCatalog, regionQuery))
    const empty = PilotPlacesResponseSchema.parse(queryPilotCatalog(orderingCatalog, emptyQuery))
  ...
  it("keeps order invariant to input order and rejects a cursor after its anchor changes", () => {
    ...
    const firstQuery = PilotQuerySchema.parse({ limit: 1 })
    const first = PilotPlacesResponseSchema.parse(queryPilotCatalog(orderingCatalog, firstQuery))
    const reversed = {
      ...orderingCatalog,
    ...
    const reordered = PilotPlacesResponseSchema.parse(queryPilotCatalog(reversed, firstQuery))
    const stale = queryPilotCatalog(
      orderingCatalog,
      PilotQuerySchema.parse({
        limit: 1,
  ...
  it("rejects a cursor when a nonmatching active menu expires", () => {
    ...
    const expiring = PilotCatalogSchema.parse({
      ...orderingCatalog,
      menus: [...orderingCatalog.menus, nonmatchingMenu],
    })
    const first = PilotPlacesResponseSchema.parse(
      queryPilotCatalog(expiring, PilotQuerySchema.parse({ query: "샐러드", limit: 1 })),
    )
    ...
    const stale = queryPilotCatalog(
      currentPilotCatalog(expiring, Date.parse("2026-09-07T00:00:00Z")),
      PilotQuerySchema.parse({ query: "샐러드", limit: 1, cursor: first.nextCursor }),
    )
  ...
  it("groups administrative aliases without losing places or mixing districts", () => {
    const responses = ["서울 강남구", "서울시 강남구", "서울특별시 강남구"].map((region) =>
      PilotPlacesResponseSchema.parse(places(`region=${encodeURIComponent(region)}&limit=100`)),
    )
    const first = responses[0]
    expect(first?.total).toBeGreaterThan(1)
    for (const response of responses) expect(response).toEqual(first)
    const regions = PilotRegionsResponseSchema.parse(places("mode=regions"))
  ...
  it("redacts source evidence and keeps page and count coherent", () => {
    const response = PilotPlacesResponseSchema.parse(places("limit=2"))
  ...
  it("paginates stably and rejects cursor reuse across queries or catalog versions", () => {
    const first = PilotPlacesResponseSchema.parse(places("limit=2"))
    const second = PilotPlacesResponseSchema.parse(places(`limit=2&cursor=${first.nextCursor}`))
    ...
    expect(
      queryPilotCatalog(
        ...
        PilotQuerySchema.parse({ limit: 2, cursor: first.nextCursor }),
      ),
  ...
  it("uses one same-menu predicate for regions, list, count and map data", () => {
    const response = PilotPlacesResponseSchema.parse(places("filter=salad_poke&ingredient=fish"))
    expect(
    ...
    const empty = PilotPlacesResponseSchema.parse(places("south=0&north=1&west=0&east=1"))
  ...
  it("keeps a semantic region aggregate independent of a selected area", () => {
    ...
    const aggregate = PilotRegionsResponseSchema.parse(places(`mode=regions&${conditions}`))
    const selectedArea = PilotPlacesResponseSchema.parse(
  ...
  it("requires explicit cooking plus ingredient and reason instead of generic meal facts", () => {
    ...
    const generic = PilotMenuSchema.parse({
      ...base,
      name: "닭국수",
      facts: {
        ...base.facts,
    ...
    expect(hasPilotMealSelectionBasis(generic)).toBe(false)
    for (const cooking of ["grilled", "steamed", "roasted"]) {
      const explicit = PilotMenuSchema.parse({
        ...generic,
        name: "닭구이",
        facts: {
          ...generic.facts,
      ...
      expect(hasPilotMealSelectionBasis(explicit)).toBe(true)
    }
  })
})
...
describe("v2 source semantics", () => {
  it("rejects legacy catalogs and unapproved media", () => {
    expect(
      PilotCatalogSchema.safeParse({ ...catalog, schemaVersion: "pilot-menu-facts-1" }).success,
    ).toBe(false)
  ...
  it("does not infer eligibility from a mismatched or absent selection reason", () => {
    const menu = catalog.menus.find((menu) => hasPilotMealSelectionBasis(menu))
    if (!menu) throw new Error("missing selected fixture")
    expect(hasPilotMealSelectionBasis({ facts: { ...menu.facts, selection_reasons: [] } })).toBe(
      false,
    )
    expect(
      hasPilotMealSelectionBasis({
        facts: {
          ...menu.facts,
          form: "rice",
          rice_base: "unknown",
