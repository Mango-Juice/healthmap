...
import { DiscoveryPlaceDtoSchema, DiscoveryPlacesResponseSchema } from "../../lib/discovery/dto"
...
const store = DiscoveryPlaceDtoSchema.parse({
...
const placesResponse = DiscoveryPlacesResponseSchema.parse({
...
describe("place HTTP handlers", () => {
  it("rejects store fields that contradict the listing kind", () => {
    ...
    expect(
      DiscoveryPlaceDtoSchema.safeParse({
        ...store,
        storeDescription: null,
        officialStoreUrl: null,
      }).success,
  ...
  it("serves store-only search and detail without menu evidence", async () => {
    ...
    expect(DiscoveryPlacesResponseSchema.parse(await list.json()).results[0]).toMatchObject({
