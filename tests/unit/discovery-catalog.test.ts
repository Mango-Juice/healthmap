...
import { currentDiscoveryCatalog, DiscoveryCatalogSchema } from "../../lib/discovery/catalog"
import { syntheticDiscoveryCatalog } from "../fixtures/discovery-catalog"

const catalog = syntheticDiscoveryCatalog
describe("Discovery menu-fact boundary", () => {
  it("removes expired source menus at request time even without regenerating the snapshot", () => {
    expect(currentDiscoveryCatalog(catalog, Date.parse("2028-01-01T00:00:00Z")).places).toEqual([])
  })
  ...
  it("rejects duplicate menu IDs and expired evidence", () => {
    expect(
      DiscoveryCatalogSchema.safeParse({ ...catalog, menus: [...catalog.menus, ...catalog.menus] })
        .success,
    ).toBe(false)
    expect(
      DiscoveryCatalogSchema.safeParse({ ...catalog, asOf: "2028-01-01T00:00:00Z" }).success,
    ).toBe(false)
  })
  ...
  it("accepts official Bon Dosirak evidence and rejects unsafe host or query variants", () => {
    ...
    expect(DiscoveryCatalogSchema.safeParse(withEvidenceUrl("not-a-url")).success).toBe(false)
