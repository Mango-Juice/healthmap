...
test("local mode wires the bounded discovery fixture and isolates the Next dist directory by port", () => {
  ...
  assert.equal(config.webServer[0].env.LOCAL_DISCOVERY_PORT, "14518")
  assert.match(config.webServer[0].env.LOCAL_TLS_ROOT, /healthmap-local-tls-/)
  ...
  assert.match(config.webServer[1].command, /local-tls-next\.mts/)
  ...
  assert.equal(config.webServer[1].env.LOCAL_TLS_ROOT, config.webServer[0].env.LOCAL_TLS_ROOT)
