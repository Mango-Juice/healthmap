import { DiscoveryPlacesResponseSchema } from "../../lib/discovery/dto"
...
test("pagination shares the total and appends matching places", async ({ page }) => {
  ...
  await page.route("**/api/places?**", async (route) => {
    ...
    const catalog = DiscoveryPlacesResponseSchema.parse(await response.json())
  ...
  await page.setViewportSize({ width: 1280, height: 800 })
  await page.goto("/")
  await expect(page.locator("[data-food-map-place-id]")).toHaveCount(1)
  const firstTotal = await page.getByLabel("검색 결과 수").innerText()
  const totalMatch = /^([0-9]+)곳 중 1곳$/u.exec(firstTotal)
  if (!totalMatch) throw new TypeError("Expected a one-item Discovery page")
  await page.getByRole("button", { name: "장소 더 보기" }).click()
  await expect(page.locator("[data-food-map-place-id]")).toHaveCount(2)
...
test("common menu notice and directions survive media excluded for another menu", async ({
  ...
  const data = DiscoveryPlacesResponseSchema.parse(await response.json())
  const original = data.results.find((candidate) => candidate.menus.length > 1)
  if (!original) throw new TypeError("Expected a Discovery result with two matching menus")
  ...
  const fixture = DiscoveryPlacesResponseSchema.parse({
  ...
  if (!result) throw new TypeError("Expected a parsed Discovery media fixture")
  await page.route("**/api/places?**", async (route) => {
  ...
  await page.setViewportSize({ width: 1280, height: 800 })
  await page.goto("/")
  const card = page.locator("[data-food-map-place-id]").first()
...
test("a late search response cannot overwrite a newer query", async ({ page, request }) => {
  ...
  release?.()
  await oldDelivery
  await expect(page.locator("[data-food-map-place-id]")).toHaveCount(0)
