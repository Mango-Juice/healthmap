...
test("the production root connects discovery, place actions, and suggestion context", async ({
  ...
  const firstResult = page.locator("[data-food-map-place-id]").first()
  await expect(firstResult).toBeVisible()
  const firstPlaceId = await firstResult.getAttribute("data-food-map-place-id")
  ...
  const marker = page.getByTestId("food-map-naver-map").getByRole("button", { name: placeName })
  await marker.click()
