...
const evidenceDirectory = ".omo/evidence/subway-salad-marker/browser"
...
for (const viewport of [
  ...
  test(`opens an honest Subway store detail on ${viewport.name}`, async ({ page, request }) => {
    ...
    await page.screenshot({
      path: `${evidenceDirectory}/subway-marker-normal-${viewport.width}x${viewport.height}.png`,
    })
    ...
    await page.screenshot({
      path: `${evidenceDirectory}/subway-marker-selected-${viewport.width}x${viewport.height}.png`,
    })
    ...
    await page.screenshot({
      path: `${evidenceDirectory}/subway-salad-results-${viewport.width}x${viewport.height}.png`,
    })
    await result.click()
    await expect(marker.locator("img[data-test-marker-icon]")).toHaveAttribute(
      "src",
    ...
    await expect(storeSummary).toContainText("메뉴 정보는 아직 확인되지 않았어요.")
    await page.screenshot({
      path: `${evidenceDirectory}/subway-salad-detail-${viewport.width}x${viewport.height}.png`,
    })
  })
}
