...
import {
  DISCOVERY_NATIONAL_VIEW,
  DISCOVERY_START_VIEW,
  discoveryBoundsAround,
} from "../../lib/discovery/viewport"
...
export function useFoodMapViewport(point: GeoPoint | undefined) {
  const [view, setView] = useState(DISCOVERY_START_VIEW)
  const [bounds, setBounds] = useState<ViewportBounds | undefined>(
    discoveryBoundsAround(DISCOVERY_START_VIEW),
  )
  ...
  useEffect(() => {
    ...
    setRegion("")
    setBounds(discoveryBoundsAround(point))
